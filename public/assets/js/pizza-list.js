...
const getPizzaList = () => {
  fetch('/api/pizzas')
    .then(response => response.json()) //get data and transform into JSON so we can use it
    .then(pizzaListArr => {
      pizzaListArr.forEach(printPizza);//run each data thru printPizza
    })
    .catch(err => {
      console.log(err);
    })
